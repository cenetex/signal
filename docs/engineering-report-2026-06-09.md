...
**Date:** 2026-06-09 · **Updated:** 2026-06-13 · **Branch:** main @ `31197b1` for original audit; remediation notes through the 2026-06-13 replay/build-flag, planned-outpost, and station-jostle slices · **Scope:** full repo audit (sim, economy, client, protocol, persistence, infra, decentralization stack)
...
- `shared/fixpoint.c` provides deterministic `sqrt/sin/cos/atan2/exp/pow` replacements and the build sets `-ffp-contract=off -fno-fast-math`. As of 2026-06-13, deterministic helpers are integrated through the authoritative `server/` and `shared/` sim surfaces covered by `make deterministic-libm`, and the `signal_replay` harness is included in the ratchet because it emits the cross-build evidence. The default replay checks cover eleven scenarios, including mining→fracture, fragment identity hash inputs, collision→death recovery, planned-outpost scaffold materialization with station construction/contract state hashed, and station-pair jostle with transient jostle velocity hashed. **Sim state is still float throughout**, so #588 is not complete, but the project now has an incremental migration path with guardrails instead of a dormant fixed-point library.
...
*Methodology: six parallel exploration passes (sim core, economy/provenance, client, protocol/persistence, build/CI/deploy, docs/on-chain) plus git-history analysis, reconciled against source. File:line references verified against main @ `31197b1`; 2026-06-13 remediation notes reconciled through the deterministic replay station-jostle slice.*
