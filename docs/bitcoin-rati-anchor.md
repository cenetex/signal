...
| Settlement checkpoints | `signal_checkpoint` computes a v1 checkpoint root over every verified station log (head hash plus full-log digest), chained to the previous root, with per-station inclusion proofs. Pass it to `build-rati-anchor-batch --settlement-checkpoint-root`. State roots and forward-apply checkpoints are not implemented. | Publish checkpoints on a cadence with discovery manifests; add state roots over settlement-owned state. | Medium. Checkpoints commit to history; they do not yet commit to derived state. |
