...
| Cold-start traffic is still the proving ground. | Local-only pressure exists, but a no-omniscience world must keep moving without a hidden global contract feed. `signal_replay --active-workers` now keeps seeded NPC workers alive, validates active NPCs/exchanged knowledge/HNN market memory over 10k ticks, has focused HNN-backed worker fixtures, and includes a gossip-courier replay fixture that fails unless a worker physically carries a remote contract/market memory into another station. | Run the WASM gate on PRs and add broader autonomy metrics before treating this as fully covered. |
...
| Determinism gates under-cover AI economy drift. | Native/WASM replay gates, PR-triggered replay CI, long probes, and active-worker/gossip/HNN replay scenarios exist. Active-worker rows hash station/NPC knowledge, job diagnostics, HNN trace bodies, worker assignment outcomes, scaffold tow state, and spatial field state. `signal.replay_ai_memory.v4` now adds branch-wide active ticks, selected/HNN diagnostic peaks, assignment ticks by job family, motion/route-support ticks, cargo/scaffold/delivery ticks, useful-outcome ticks, station-held remote contract counts, station-held remote market-memory counts, and field load/noise/margin/SNR diagnostics. | Active-worker replay is still a bounded probe rather than a full no-omniscience economy soak. | Add a full no-omniscience outcome soak once starter traffic can reliably complete work without hidden global pressure. |
...
| No-omniscience proof | The named replay gate now includes a focused physical gossip-courier scenario with remote station-held contract/memory counters. The broader soak still needs longer fresh-world autonomy metrics: no global contract flood, local pressure only, repeated physical memory spread, and measurable completed work. |
...
- a deterministic fresh-world soak with no global contract flood still produces
  ...
  transport is now covered in both C and replay gates by
  `test_neural_worker_physically_transports_contract_memory_between_stations`
  and the `worker-gossip-courier` replay scenario.
