...
#define HNN_FEATURE_VALUE_KEY_SEED_BASE 3000ull
...
#define HNN_CONTRACT_SEED \
    ((((uint64_t)HNN_ACTION_KEY_SEED_BASE) << 48) | \
     (((uint64_t)HNN_FEATURE_VALUE_KEY_SEED_BASE) << 16) | \
 ...
 * adjacent deterministic level anchors belonging to that feature. Values are
 * clipped to [-1, 1], non-finite values become zero, and all feature/value
 * codes are bundled into one unit vector.
 */
