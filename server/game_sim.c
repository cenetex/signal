...
static float apply_scaffold_tow_physics(server_player_t *sp,
                                        scaffold_t *sc,
                                        float dt);
static bool find_nearest_open_slot(const station_t *st, vec2 pos,
                                   module_type_t type,
...
bool world_tow_link_canonical_before(const tow_link_t *a,
    ...
    if (a->source.part != b->source.part)
        return a->source.part < b->source.part;
...
int world_tow_link_count_for_source(const world_t *w, entity_ref_t source,
    ...
    return world_tow_collect_targets(w, source, profile, NULL, 0);
}

static bool world_tow_link_capacity_allows(const world_t *w,
                                           entity_ref_t source,
                                           tow_profile_t profile,
                                           const tow_link_t *replaced) {
    if (source.kind == ENTITY_KIND_STATION_MODULE)
        return profile == TOW_PROFILE_MODULE_POD;
    if (source.kind != ENTITY_KIND_SHIP) return false;
    ...
    for (int i = 0; i < MAX_TOW_LINKS; i++) {
        const tow_link_t *link = &w->tow_links[i];
        if (!link->active || link == replaced ||
            !entity_ref_equal(link->source, source)) continue;
        switch ((tow_profile_t)link->profile) {
        case TOW_PROFILE_SHIP_FRAGMENT: fragments++; break;
        case TOW_PROFILE_SHIP_POD:      pods++; break;
        case TOW_PROFILE_SHIP_SCAFFOLD: scaffolds++; break;
        default: break;
        }
    }
...
static bool world_tow_link_store(world_t *w, entity_ref_t source,
    ...
    if (!world_tow_link_capacity_allows(w, source, profile, link)) return false;
    if (!link) {
        for (int i = 0; i < MAX_TOW_LINKS; i++) {
...
bool world_ship_cached_views_valid(const world_t *w) {
    if (!w) return false;
    for (int p = 0; p < MAX_PLAYERS; p++) {
        const ship_t *resolved = world_ship_resolve_const(
            w, w->players[p].ship_ref);
        if (w->players[p].ship != resolved) return false;
    }
    for (int n = 0; n < MAX_NPC_SHIPS; n++) {
        const ship_t *resolved = world_ship_resolve_const(
            w, w->npc_ships[n].ship_ref);
        if (w->npc_ships[n].ship != resolved) return false;
    }
    for (int a = 0; a < MAX_SHIP_ASSETS; a++) {
        const ship_asset_t *asset = &w->ship_assets[a];
        const ship_t *expected = NULL;
        if (asset->active && asset->status == SHIP_ASSET_STATUS_ASSIGNED)
            expected = world_ship_resolve_const(w, asset->live_ship_ref);
        else if (asset->active)
            expected = &asset->stored_ship;
        if (asset->ship != expected) return false;
    }
    return true;
}
...
static int ship_towed_fragment_slot(world_t *w, entity_ref_t source,
                                    int asteroid_idx) {
    if (!w || entity_ref_is_none(source) || asteroid_idx < 0 ||
        asteroid_idx >= MAX_ASTEROIDS) return -1;
    entity_ref_t target = world_entity_ref_for_slot(
        w, ENTITY_KIND_ASTEROID, asteroid_idx, -1);
    const tow_link_t *link = world_tow_link_for_target_const(w, target);
    return link && entity_ref_equal(link->source, source) &&
                   link->profile == TOW_PROFILE_SHIP_FRAGMENT
        ? (int)link->slot : -1;
}

/* The relationship pool owns live fragment custody. Target bindings and ship
 * arrays are compact projections for physics, saves, and wire compatibility. */
static bool player_attach_fragment(world_t *w, server_player_t *sp,
    ...
    int slot = ship_towed_fragment_slot(w, sp->ship_ref, asteroid_idx);
    if (slot < 0)
        slot = world_tow_link_count_for_source(
            w, sp->ship_ref, TOW_PROFILE_SHIP_FRAGMENT);
    entity_ref_t target = world_entity_ref_for_slot(
        w, ENTITY_KIND_ASTEROID, asteroid_idx, -1);
    return world_tow_link_set(
...
static int ship_towed_pod_slot(world_t *w, entity_ref_t source, int pod_idx) {
    if (!w || entity_ref_is_none(source) || pod_idx < 0 ||
        pod_idx >= MAX_CARGO_PODS) return -1;
    entity_ref_t target = world_entity_ref_for_slot(
        w, ENTITY_KIND_CARGO_POD, pod_idx, -1);
    const tow_link_t *link = world_tow_link_for_target_const(w, target);
    return link && entity_ref_equal(link->source, source) &&
...
/* Mutate the player/pod relationship through the pool. Target bindings and
 * ship lists are rebuilt projections retained for physics, save, and wire
 * compatibility. */
...
static void world_tow_rebuild_projections(world_t *w) {
    ...
    /* Projection order is explicit and independent of relationship-pool slot
     * reuse. This keeps save/wire hold slots stable after a middle link is
     * released while leaving tow_links as the only relationship authority. */
    tow_link_t *ordered[MAX_TOW_LINKS];
    int count = 0;
    for (int i = 0; i < MAX_TOW_LINKS; i++) {
...
static void reconcile_tractor_relationships(world_t *w) {
    /* Runtime ticks validate the relation pool and rebuild read projections;
     * they never import target/ship mirrors back into authority. Legacy
     * imports happen only through the explicit world_tow_links_reconcile()
     * load/test boundary. */
    world_tow_links_refresh(w);
}
...
static bool charge_station_owned_pod_if_due(world_t *w,
    ...
    emit_event(w, (sim_event_t){
        ...
        .buy = { .station = station_idx,
                 ...
                 .grade = (uint8_t)cargo_pod_manifest_best_grade(pod),
...
static bool is_already_towed(world_t *w, const server_player_t *sp,
                             int asteroid_idx);
...
static bool is_already_towed(world_t *w, const server_player_t *sp,
                             int asteroid_idx) {
    return sp && ship_towed_fragment_slot(w, sp->ship_ref, asteroid_idx) >= 0;
}
...
static void step_fragment_collection(world_t *w, server_player_t *sp, float dt) {
    ...
    for (int i = 0; i < MAX_ASTEROIDS; i++) {
        ...
        if (is_already_towed(w, sp, i) || asteroid_has_tractor(a)) continue;
...
static void step_predicted_towed_body_forces(world_t *w, server_player_t *sp,
    ...
    if (sc_idx >= 0 && sc_idx < MAX_SCAFFOLDS) {
        ...
        if (sc->active && sc->state == SCAFFOLD_TOWING)
            (void)apply_scaffold_tow_physics(sp, sc, dt);
    }
}
...
static void place_towed_scaffold(world_t *w, server_player_t *sp) {
    ...
    if (sp->input.place_target_station >= 0) {
        ...
        if (s >= 0 && s < MAX_STATIONS && station_is_active(&w->stations[s])) {
            ...
            if (station_placement_validate(
                    st, sc->module_type, ring, slot,
                    STATION_PLACEMENT_SCAFFOLD) == STATION_PLACEMENT_OK) {
    ...
    if (sc->module_type == MODULE_SIGNAL_RELAY) {
        ...
        for (int s = SIGNAL_FIRST_OUTPOST_INDEX; s < MAX_STATIONS; s++) {
            ...
            if (chosen_ring < 0 ||
                station_placement_validate(
                    st, sc->module_type, chosen_ring, chosen_slot,
                    STATION_PLACEMENT_SCAFFOLD) != STATION_PLACEMENT_OK) {
                chosen_ring = 1;
                chosen_slot = 0;
            }
    ...
    for (int s = SIGNAL_FIRST_OUTPOST_INDEX; s < MAX_STATIONS; s++) {
        ...
        if (!station_is_active(st)) continue;
        int ring, slot;
        if (find_nearest_open_slot(st, sc->pos, sc->module_type,
                                   &ring, &slot)) {
            sc->state = SCAFFOLD_SNAPPING;
            sc->placed_station = s;
            sc->placed_ring = ring;
                ...
                if (sp->input.mine && sp->beam_active) {
                    for (int si = 0; si < MAX_SCAFFOLDS; si++) {
                        ...
                        for (int s = SIGNAL_FIRST_OUTPOST_INDEX; s < MAX_STATIONS; s++) {
                            ...
                            if (!station_is_active(st)) continue;
                            int ring, slot;
                            if (find_nearest_open_slot(
                                    st, sc->pos, sc->module_type,
                                    &ring, &slot)) {
                                sc->state = SCAFFOLD_SNAPPING;
                                sc->placed_station = s;
                                sc->placed_ring = ring;
    ...
    if (sp->input.add_plan && !w->player_only_mode) {
        ...
        if (s >= SIGNAL_FIRST_OUTPOST_INDEX && s < MAX_STATIONS && station_exists(&w->stations[s])
            ...
            station_placement_status_t placement = station_placement_validate(
                st, type, ring, slot, STATION_PLACEMENT_PLAN);
            bool ring_unlocked = ring <= station_max_plannable_ring(st);
            bool type_unlocked = sp->ship && module_unlocked_for_player(
                sp->ship->unlocked_modules, type);
            if (!taken && !over_cap && ring_unlocked && type_unlocked &&
                placement == STATION_PLACEMENT_OK) {
                if (existing >= 0) {
    ...
    for (int s = 0; s < MAX_STATIONS; s++) {
        ...
        if (!need.active && !has_ore_contract && station_has_module(st, MODULE_FURNACE)) {
            ...
            for (int c = 0; c < COMMODITY_RAW_ORE_COUNT; c++) {
                station_supply_need_t supply = station_supply_need_for(
                    st, (commodity_t)c);
                if (supply.should_open && supply.severity > worst_deficit) {
                    worst_deficit = supply.severity;
                    worst_ore = c;
                }
            }
            if (worst_ore >= 0) {
        ...
        if (!need.active && !has_production_contract) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                station_supply_need_t supply = station_supply_need_for(
                    st, checks[j].ingot);
                if (supply.should_open && supply.deficit > worst_deficit) {
                    worst_deficit = supply.deficit;
                    worst_idx = j;
                }
            }
            if (worst_idx >= 0) {
        ...
        if (!need.active && !has_production_contract
            ...
            if (supply.should_open) {
                float deficit = supply.deficit;
...
static bool shipyard_chainless_unit_trusted(const world_t *w,
    ...
    cargo_receipt_station_evaluation_t evaluated =
        cargo_receipt_evaluate_at_station(w, station_idx, unit, NULL);
    return evaluated.accepted &&
...
static bool ship_birth_bytes_any(const uint8_t *bytes, size_t len) {
...
static void ship_birth_hash_u32(sha256_ctx_t *ctx, uint32_t v) {
    uint8_t b[4] = {
        (uint8_t)(v & 0xffu),
        (uint8_t)((v >> 8) & 0xffu),
        (uint8_t)((v >> 16) & 0xffu),
...
static bool ship_birth_domain_hash(const char *domain,
                                   const uint8_t pubs[][32],
                                   size_t count,
                                   uint8_t out[32]) {
    uint8_t root[32];
    sha256_ctx_t ctx;
    if (!domain || !out || !pubs || count == 0) return false;
    if (!hash_merkle_root(pubs, count, root)) return false;
    sha256_init(&ctx);
    sha256_update(&ctx, domain, strlen(domain));
    ship_birth_hash_u32(&ctx, (uint32_t)count);
...
static bool ship_birth_ship_tows_fragment(const ship_t *ship, int idx) {
    ...
    if (count > cap) count = cap;
    for (int t = 0; t < count; t++) {
        if (ship->towed_fragments[t] == idx) return true;
    }
    return false;
}
...
static int ship_birth_pick_fragment(world_t *w, int station_idx,
    ...
    float range = st->signal_range > 0.0f ? st->signal_range : 9000.0f;
    ...
    for (int i = 0; i < MAX_ASTEROIDS; i++) {
        ...
        seen++;
        if (rand_int(&w->rng, 1, seen) == 1)
            pick = i;
    }
    return pick;
}

static int ship_birth_find_fragment_const(const world_t *w, int station_idx,
                                          commodity_t commodity,
                                          int avoid_a, int avoid_b) {
    if (!w || station_idx < 0 || station_idx >= MAX_STATIONS) return -1;
    const station_t *st = &w->stations[station_idx];
...
static bool ship_birth_station_has_fragments(const world_t *w,
                                             int station_idx) {
    int fe = ship_birth_find_fragment_const(w, station_idx,
                                            COMMODITY_FERRITE_ORE, -1, -1);
    int cu = ship_birth_find_fragment_const(w, station_idx,
                                            COMMODITY_CUPRITE_ORE, fe, -1);
    int cr = ship_birth_find_fragment_const(w, station_idx,
                                            COMMODITY_CRYSTAL_ORE, fe, cu);
    return fe >= 0 && cu >= 0 && cr >= 0;
}

static bool ship_birth_assembly_start(world_t *w, int station_idx,
                                      int build_idx) {
    if (!w || station_idx < 0 || station_idx >= MAX_STATIONS ||
    ...
    ship_birth_assembly_t *birth =
        &w->ship_birth_assemblies[station_idx][build_idx];
    if (birth->active) return true;

    int fe = ship_birth_pick_fragment(w, station_idx, COMMODITY_FERRITE_ORE,
                                      -1, -1);
    int cu = ship_birth_pick_fragment(w, station_idx, COMMODITY_CUPRITE_ORE,
                                      fe, -1);
    int cr = ship_birth_pick_fragment(w, station_idx, COMMODITY_CRYSTAL_ORE,
                                      fe, cu);
    if (fe < 0 || cu < 0 || cr < 0) return false;

    memset(birth, 0, sizeof(*birth));
    birth->active = true;
    birth->fragment_slots[SHIP_BIRTH_FERRITE] = (int16_t)fe;
    birth->fragment_slots[SHIP_BIRTH_CUPRITE] = (int16_t)cu;
    birth->fragment_slots[SHIP_BIRTH_CRYSTAL] = (int16_t)cr;
    vec2 target = v2(0.0f, 0.0f);
    ...
    birth->target = v2_scale(target, 1.0f / (float)SHIP_BIRTH_FRAGMENT_COUNT);
    for (int i = 0; i < SHIP_BIRTH_FRAGMENT_COUNT; i++) {
        int idx = birth->fragment_slots[i];
        birth->start_dist[i] =
            fixp_sqrtf(v2_dist_sq(w->asteroids[idx].pos, birth->target));
        if (birth->start_dist[i] < 1.0f) birth->start_dist[i] = 1.0f;
    }
    SIM_LOG("[ship-birth] station %d claimed fragments fe=%d cu=%d cr=%d\n",
            station_idx, fe, cu, cr);
    return true;
}

static bool ship_birth_assembly_still_valid(const world_t *w,
                                            const ship_birth_assembly_t *birth) {
    if (!w || !birth || !birth->active) return false;
    for (int i = 0; i < SHIP_BIRTH_FRAGMENT_COUNT; i++) {
        int idx = birth->fragment_slots[i];
...
static bool ship_birth_assembly_step(world_t *w, int station_idx,
                                     int build_idx, float dt,
                                     ship_birth_material_t *out_material) {
    station_t *st = &w->stations[station_idx];
    pending_ship_build_t *build = &st->pending_ship_builds[build_idx];
    ship_birth_assembly_t *birth =
        &w->ship_birth_assemblies[station_idx][build_idx];
    ...
    for (int i = 0; i < SHIP_BIRTH_FRAGMENT_COUNT; i++) {
        int idx = birth->fragment_slots[i];
        asteroid_t *a = &w->asteroids[idx];
        vec2 to_target = v2_sub(birth->target, a->pos);
        float dist = v2_len(to_target);
        float arrive = fmaxf(96.0f, a->radius + 14.0f);
        float ratio = 1.0f - ship_birth_clampf(dist / birth->start_dist[i],
                                               0.0f, 1.0f);
        ...
        vec2 dir = v2_scale(to_target, 1.0f / fmaxf(dist, 0.001f));
        float speed = ship_birth_clampf(120.0f + dist * 0.9f,
                                        180.0f, 1100.0f);
        float step = fminf(dist, speed * dt);
        a->pos = v2_add(a->pos, v2_scale(dir, step));
        a->vel = v2_scale(dir, speed);
        a->spin *= 0.985f;
        a->smelt_progress = ship_birth_clampf(ratio, 0.0f, 0.98f);
        a->net_dirty = true;
    }
    build->build_progress = ship_birth_clampf(progress, build->build_progress,
                                              0.99f);
    if (arrived < SHIP_BIRTH_FRAGMENT_COUNT) return false;
    build->build_progress = 1.0f;
    return ship_birth_material_from_fragments(w, birth, station_idx,
                                              out_material);
}
...
static void ship_birth_apply_material(ship_asset_t *asset,
    ...
    memcpy(asset->birth_fragment_pubs, material->fragment_pubs,
...
static bool shipyard_consume_hull_materials_atomic(
    ...
    int frames, int lasers, int tractors,
    bool include_towed_pods, int yard_idx) {
    if (!w || !st || frames < 0 || lasers < 0 || tractors < 0 ||
        frames + lasers + tractors <= 0 ||
        frames + lasers + tractors > CHAIN_LOG_BATCH_MAX_EVENTS) {
        return false;
    }
    ...
    bool frames_ok = shipyard_stage_one_material(
            w, st, station_idx, ship, COMMODITY_FRAME,
            frames, include_towed_pods, yard_idx, plan);
    bool lasers_ok = frames_ok && shipyard_stage_one_material(
            w, st, station_idx, ship, COMMODITY_LASER_MODULE,
            lasers, include_towed_pods, yard_idx, plan);
    bool tractors_ok = lasers_ok && shipyard_stage_one_material(
            w, st, station_idx, ship, COMMODITY_TRACTOR_MODULE,
            tractors, include_towed_pods, yard_idx, plan);
    bool staged = frames_ok && lasers_ok && tractors_ok;
    if (!staged || plan->unit_count !=
            (size_t)(frames + lasers + tractors) ||
...
static bool shipyard_queue_hull_build(
    ...
    if (!use_birth_assembly &&
        !shipyard_consume_hull_materials_atomic(
            w, st, station_idx,
            include_towed_pods ? sp->ship : NULL,
    ...
    if (use_birth_assembly &&
        !ship_birth_assembly_start(w, station_idx, idx)) {
        st->pending_ship_build_count--;
        memset(&st->pending_ship_builds[idx], 0,
               sizeof(st->pending_ship_builds[idx]));
        memset(&w->ship_birth_assemblies[station_idx][idx], 0,
               sizeof(w->ship_birth_assemblies[station_idx][idx]));
        return false;
    }
    return true;
}
...
bool shipyard_queue_station_hull_request(world_t *w, int requester_station,
    ...
    for (int s = 0; s < MAX_STATIONS; s++) {
        ...
        bool can_material = shipyard_can_commission_hull_from_ship(
            w, st, s, NULL, hull_class, false);
        bool can_birth = !can_material && station_active_shipyard_count(st) >= 1 &&
                         ship_birth_station_has_fragments(w, s);
        if (!can_material && !can_birth) continue;
...
static bool find_nearest_open_slot(const station_t *st, vec2 pos,
    ...
    for (int ring = 1; ring <= STATION_NUM_RINGS; ring++) {
        ...
        /* Check if any slot on this ring satisfies the canonical physical
         * placement contract for this scaffold type. */
        int slots = STATION_RING_SLOTS[ring];
        bool any_valid = false;
        for (int slot = 0; slot < slots; slot++) {
            if (station_placement_validate(
                    st, type, ring, slot, STATION_PLACEMENT_SCAFFOLD) ==
    ...
    for (int slot = 0; slot < slots; slot++) {
        if (station_placement_validate(
                st, type, best_ring, slot, STATION_PLACEMENT_SCAFFOLD) !=
...
static bool finalize_scaffold_placement(world_t *w, scaffold_t *sc) {
    ...
    station_placement_status_t placement = station_placement_validate(
        st, sc->module_type, sc->placed_ring, sc->placed_slot,
        STATION_PLACEMENT_SCAFFOLD);
    if (placement != STATION_PLACEMENT_OK) return false;
    ...
    for (int i = 0; i < MAX_SCAFFOLDS; i++) {
        ...
        if (sc->state == SCAFFOLD_SNAPPING) {
            ...
            if (dist < SCAFFOLD_SNAP_ARRIVE) {
                ...
                sc->pos = target;
                if (!finalize_scaffold_placement(w, sc)) {
                    sc->state = SCAFFOLD_LOOSE;
                    sc->placed_station = -1;
                    sc->placed_ring = -1;
                    sc->placed_slot = -1;
...
void world_sim_step(world_t *w, float dt) {
    ...
    /* Raw ship pointers are derived caches only. World copies, reconnect
     * transfers, and save staging may move controller records between ticks;
     * resolve every cache from its generation-checked handle before systems
...
signed_action_result_t signed_action_verify(const world_t *w, int player_idx,
    ...
    uint64_t nonce       = wire_read_u64_le(&data[1]);
    uint8_t  action_type = data[9];
    uint16_t payload_len = wire_read_u16_le(&data[10]);
